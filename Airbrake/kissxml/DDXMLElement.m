...
- (id)initWithName:(NSString *)name
{
	...
	xmlNodePtr node = xmlNewNode(NULL, [name ddxml_xmlChar]);
	if (node == NULL)
	{
		return nil;
	}
                          ...
                          uri:(NSString *)uri
{
	...
	const xmlChar *xmlName      = [name ddxml_xmlChar];
	const xmlChar *xmlLocalName = [localName ddxml_xmlChar];
	const xmlChar *xmlUri       = [uri ddxml_xmlChar];
...
- (NSArray *)elementsForName:(NSString *)name
{
	...
	if ([prefix length] > 0)
	{
		...
		xmlNsPtr ns = xmlSearchNs(node->doc, node, [prefix ddxml_xmlChar]);
		if (ns)
		{
...
- (DDXMLNode *)namespaceForPrefix:(NSString *)prefix
{
	...
	else
	{
		...
		if (ns)
		{
			const xmlChar *xmlPrefix = [prefix ddxml_xmlChar];
			do
			{
