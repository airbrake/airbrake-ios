...
{
	xmlAttrPtr attr = xmlNewProp(NULL, [name ddxml_xmlChar], [stringValue ddxml_xmlChar]);
...
+ (id)namespaceWithName:(NSString *)name stringValue:(NSString *)stringValue
{
	...
	const xmlChar *xmlName = [name length] > 0 ? [name ddxml_xmlChar] : NULL;
	
	xmlNsPtr ns = xmlNewNs(NULL, [stringValue ddxml_xmlChar], xmlName);
...
{
	xmlNodePtr procInst = xmlNewPI([name ddxml_xmlChar], [stringValue ddxml_xmlChar]);
...
{
	xmlNodePtr comment = xmlNewComment([stringValue ddxml_xmlChar]);
...
{
	xmlNodePtr text = xmlNewText([stringValue ddxml_xmlChar]);
...
- (void)setName:(NSString *)name
{
	...
	xmlNodeSetName((xmlNodePtr)genericPtr, [name ddxml_xmlChar]);
}
...
- (void)setStringValue:(NSString *)string
{
	...
	if (IsXmlNodePtr(genericPtr))
	{
		...
		xmlChar *escapedString = xmlEncodeSpecialChars(node->doc, [string ddxml_xmlChar]);
...
- (void)setURI:(NSString *)URI
{
	...
	if (IsXmlNodePtr(genericPtr))
	{
		...
		if (URI)
		{
			...
			xmlNsPtr ns = xmlNewNs(NULL, [URI ddxml_xmlChar], NULL);
			ns->next = node->nsDef;
			node->nsDef = ns;
			node->ns = ns;
		}
...
- (NSArray *)nodesForXPath:(NSString *)xpath error:(NSError **)error
{
	...
	xpathObj = xmlXPathEvalExpression([xpath ddxml_xmlChar], xpathCtx);
...
+ (NSError *)lastError
{
	...
	if(lastErrorValue)
	{
		...
		NSString *errMsg = [[NSString stringWithFormat:@"%s", lastError.message] ddxml_stringByTrimming];
...
- (void)setName:(NSString *)name
{
	...
	ns->prefix = xmlStrdup([name ddxml_xmlChar]);
}
...
- (void)setStringValue:(NSString *)string
{
	...
	ns->href = xmlEncodeSpecialChars(NULL, [string ddxml_xmlChar]);
}
...
- (void)setStringValue:(NSString *)string
{
	...
	{
		xmlChar *escapedString = xmlEncodeSpecialChars(attr->doc, [string ddxml_xmlChar]);
		xmlNodeSetContent((xmlNodePtr)attr, escapedString);
		xmlFree(escapedString);
	}
	else
	{
		xmlNodePtr text = xmlNewText([string ddxml_xmlChar]);
		attr->children = text;
	}
}
...
- (void)setURI:(NSString *)URI
{
	...
	if (URI)
	{
		...
		const xmlChar *uri = [URI ddxml_xmlChar];
...
- (NSString *)URI
{
	...
	if ([prefix length] > 0)
	{
		xmlNsPtr ns = xmlSearchNs(attr->doc, attr->parent, [prefix ddxml_xmlChar]);
		if (ns && ns->href)
		{
